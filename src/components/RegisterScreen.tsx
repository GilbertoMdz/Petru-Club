...
const RegisterScreen: React.FC<RegisterProps> = ({ onSuccess, onBack }) => {
  ...
  const fieldClass = (hasValue: boolean, isValid: boolean) => {
    if (!hasValue) return "bg-white/70 border-black/10";
    if (isValid) return "bg-white border-black/20";
    return "bg-white/70 border-red-300";
  };
  ...
  return (
    ...
    >
      {/* Fondo: blobs suaves (tus originales) */}
      ...
      {/* Fondo: bolitas animadas como en Hero */}
      ...
      <div className="relative z-10 w-full max-w-md flex flex-col">
        ...
        <div className="px-6">
          <button
            onClick={onBack}
            className="inline-flex items-center gap-2 rounded-full px-3 py-1.5 border border-black/10 text-black/70 hover:bg-black/5 transition text-sm bg-white/40 backdrop-blur-sm"
          >
        ...
        <main className="flex-1 overflow-y-auto px-6 pb-16">
          ...
          <form onSubmit={submit} className="mt-8 space-y-5 text-left">
            ...
            <div className="relative">
              <User
                className={`absolute left-4 top-1/2 -translate-y-1/2 h-5 w-5 ${
                  !name ? "text-black/80" : nameOk ? "text-black" : "text-red-500"
                }`}
              />
              <input
                type="text"
            ...
            <div className="relative">
              <Mail
                className={`absolute left-4 top-1/2 -translate-y-1/2 h-5 w-5 ${
                  !email ? "text-black/80" : emailOk ? "text-black" : "text-red-500"
                }`}
              />
              <input
                type="email"
            ...
            <div className="relative">
              <Phone
                className={`absolute left-4 top-1/2 -translate-y-1/2 h-5 w-5 ${
                  !tel ? "text-black/80" : telOk ? "text-black" : "text-red-500"
                }`}
              />
              <input
                type="tel"
            ...
            <div className="relative">
              <Calendar
                className={`absolute left-4 top-1/2 -translate-y-1/2 h-5 w-5 ${
                  !dob ? "text-black/80" : dobOk ? "text-black" : "text-red-500"
                }`}
              />
              <input
                type="date"
            ...
            <div className="relative">
              <Lock
                className={`absolute left-4 top-1/2 -translate-y-1/2 h-5 w-5 ${
                  !pass ? "text-black/80" : passOk ? "text-black" : "text-red-500"
                }`}
              />
              <input
            ...
            <div className="relative">
              <Lock
                className={`absolute left-4 top-1/2 -translate-y-1/2 h-5 w-5 ${
                  !confirm ? "text-black/80" : confirmOk ? "text-black" : "text-red-500"
                }`}
              />
              <input
